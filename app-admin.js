...
Object.assign(App, {
    getManagedUsers() {
        ...
        (this.data.profileDirectory || []).forEach(addUser);
        (this.data.accessList || []).forEach(addUser);
        (this.data.userDirectory || []).forEach(addUser);

        if (this.user) {
            addUser({
                id: this.user.uid,
                uid: this.user.uid,
                name: this.userProfile?.name || this.user.displayName || 'Administrador',
                email: this.user.email || ADMIN_EMAIL
            });
        }
    ...
    renderAdminView() {
        ...
        if (!users.length) html += `<div class="p-12 text-center text-slate-400"><i class="fa-solid fa-users text-3xl mb-3"></i><p>Nenhum usuário localizado.</p><p class="text-xs mt-2">Verifique se as regras atualizadas do Firestore foram publicadas.</p></div>`;
        users.forEach(user => {
        ...
        return `${html}</div></section><div class="mt-5 card-surface p-5 border-l-4 border-amber-400"><h2 class="font-black text-slate-800"><i class="fa-solid fa-shield-halved text-amber-500 mr-2"></i>Segurança</h2><p class="text-sm text-slate-500 mt-2">O painel reúne usuários do diretório novo e também perfis criados antes desta atualização. Para permitir essa leitura administrativa, publique o arquivo <code class="bg-slate-100 px-1.5 py-0.5 rounded">firestore.rules</code> atualizado no Firebase.</p></div>`;
    },

    async saveUserPermissions(uid) {
        if (!this.isAdmin()) return;
        const user = this.getManagedUsers().find(item => item.id === uid);
